...
public class GameApplication {
				...
				while (isAppRunning) {
					if (selectedButtons.size() >= 2) {
						...
						Display.getDefault().asyncExec(new Runnable() {
							@Override
							public void run() {
								ButtonWithNumber first = selectedButtons.get(0);
								ButtonWithNumber second = selectedButtons.get(1);
								if (first.getNumber() == second.getNumber()) {
									first.getButton().setEnabled(false);
									second.getButton().setEnabled(false);
									pairs++;
									if (pairs == level.getLines() * level.getColums() / 2) {
										winLabel.setText("YOU WON!! (" + numberOfMismatches + " mismaches in total)");
										startButton.setEnabled(true);
									}

								} else {
									first.getButton().setText("");
									second.getButton().setText("");
									numberOfMismatches++;
								}
	...
	private void createButtons(Composite buttons) {
		...
		fd_winLabel.right = new FormAttachment(0, 550);
			...
			for (int j = 0; j < columns; j++) {
				...
				button.addSelectionListener(new SelectionListener() {
					...
					public void widgetSelected(SelectionEvent e) {
						...
						if (!selectedButtons.contains(buttonWithNumber)) {
